window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/**
		* Fallback descriptor for calls whose settled form carries no persisted
		* meta: nested dispatches (run_code's SDK tools) project presentation
		* meta only for top-level calls, so rebuild the descriptor from the
		* call's own arguments when meta is absent. Malformed or partial JSON
		* declines to undefined — the caller keeps its quiet fallback.
		* @param block - the raw call block (running or settled form).
		* @returns the narrowed descriptor, or undefined when unusable.
		*/
		function visualizeMetaFromArgs(block) {
			...
			const { fragment, title, mode } = parsed;
